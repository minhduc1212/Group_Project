...
Đây là bộ tài liệu chuẩn hóa cho dự án, dùng chung cho toàn team (6 người: Backend, Frontend, AI Engineer, Cyber Security, cùng các vai trò kiêm nhiệm). Mọi thành viên **bắt buộc đọc** các mục `01-workflow` và `02-standards` trước khi push code đầu tiên.
...
- [Hướng Dẫn Vận Hành & Luồng (Explanation)](explain.md) — **Giải thích toàn bộ luồng hoạt động, ví dụ từng EventType (Travel, Dining, Entertainment...), DeepSeek API**
- [Tech Stack](docs/00-overview/tech-stack.md) — Công nghệ sử dụng & lý do lựa chọn (React, NestJS, Postgres, Redis, DeepSeek API)
...
- [Coding Conventions](docs/02-standards/coding-conventions.md) — Style code Backend/Frontend/AI
- [Naming Conventions](docs/02-standards/naming-conventions.md) — Đặt tên biến, hàm, class, file, DB, env
- [API Design Guide](docs/02-standards/api-design-guide.md) — Chuẩn REST endpoint, domain, versioning
- [Documentation Guide](docs/02-standards/documentation-guide.md) — Quy định viết docs, README, JSDoc/Docstring
- [Testing Guide](docs/02-standards/testing-guide.md) — Chuẩn viết Unit Test / Integration Test

### 03 — Kiến trúc hệ thống (Architecture)
- [System Architecture](docs/03-architecture/system-architecture.md) — Sơ đồ tổng thể hệ thống
- [Database Schema](docs/03-architecture/database-schema.md) — Thiết kế bảng dữ liệu chính (Prisma schema, EventType, StopCategory, Invitation, metadata JSON)
- [AI Agent Architecture](docs/03-architecture/ai-agent-architecture.md) — Kiến trúc Multi-Agent (LangGraph, EventType routing, DeepSeek API)
...
- [Person 1 — Backend Dev A (Core Domain: Auth + Event/Plan/Vote/Invitation)](docs/04-tasks/person-1-backend-core.md)
- [Person 2 — Backend Dev B (Platform: External API + Notification/Export + Admin)](docs/04-tasks/person-2-backend-platform.md)
- [Person 3 — AI Engineer (Multi-Agent System & DeepSeek API)](docs/04-tasks/person-3-ai-engineer.md)
...
- [Person 6 — Cyber Security & DevOps](docs/04-tasks/person-6-security-devops.md)

### 05 — Bảo mật (Security)
- [Security Guidelines](docs/05-security/security-guidelines.md) — Checklist bảo mật, prompt injection, secrets management
...
- [.env.example](.env.example) — Mẫu biến môi trường chuẩn (bao gồm DeepSeek API key & settings)
...
2. Trước khi code → đọc `02-standards` liên quan (coding convention theo stack của mình).
