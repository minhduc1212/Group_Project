...
% ==========================================
\subsection{AI Reliability: Four-Tier Grounding \& Hallucination Mitigation Pipeline}
\label{sec:hallucination_mitigation}

A critical vulnerability of LLMs in travel planning is \textbf{hallucination}---inventing non-existent venues (``phantom POIs''), obsolete opening hours, or unrealistic transit routes. TripMate AI eliminates hallucinations through a \textbf{Four-Tier Deterministic Grounding Pipeline} (Figure~\ref{fig:hallucination_pipeline}):
...
\node[tierbox] (t1) {\textbf{Tier 1: Type-Safe Schema}\\[0.02cm]{\tiny Pydantic v2 Output Guard}\\[0.01cm]{\tiny\color{accentteal}No free-text hallucination}};
\node[tierbox, right=of t1] (t2) {\textbf{Tier 2: Live Grounding}\\[0.02cm]{\tiny Places API \& OpenStreetMap}\\[0.01cm]{\tiny\color{primaryblue}Verify Place ID \& Status}};
\node[tierbox, right=of t2] (t3) {\textbf{Tier 3: Spatio-Temporal}\\[0.02cm]{\tiny Deterministic Clustering}\\[0.01cm]{\tiny\color{accentteal}Radius $\le 2.5$\,km, transit $\le 20$\,min}};
\node[tierbox, right=of t3] (t4) {\textbf{Tier 4: Human Feedback}\\[0.02cm]{\tiny Stop Voting \& 1-Click Swap}\\[0.01cm]{\tiny\color{primaryblue}Pre-fetched verified backups}};
...
\vspace{-0.25cm}
\caption{TripMate AI four-tier deterministic hallucination mitigation and live grounding pipeline}
\label{fig:hallucination_pipeline}
\end{figure}

\begin{itemize}[leftmargin=1.2em]
\item \textbf{Tier 1 (Strict Schema Enforcement):} LangChain \texttt{with\_structured\_output()} binds DeepSeek-V4 to Pydantic v2 schemas, structurally prohibiting free-text drift and enforcing typed attributes (categories, coordinates, budgets, durations).
\item \textbf{Tier 2 (Live API Grounding):} Candidate venues are matched against Google Places API and OpenStreetMap to verify canonical \texttt{place\_id}, active operational status, and compatible opening hours. Unverified venues immediately fall back to local database records.
\item \textbf{Tier 3 (Deterministic Spatio-Temporal Checks):} Python algorithms (Haversine formula and Mapbox Matrix API) deterministically cluster stops within a $2.0$--$2.5$\,km radius with transit times $\le 20$\,minutes, preventing spatial hallucinations.
\item \textbf{Tier 4 (Human-in-the-Loop Feedback):} Granular stop voting (UP/DOWN) empowers members to reject suggestions, triggering instantaneous 1-click swaps with pre-fetched, API-verified backup venues.
\end{itemize}
...
% ==========================================
\section{Software Engineering Foundations \& Non-Functional Requirements (NFRs)}
\label{sec:nfr}

\subsection{Latency Budget \& Multi-Turn Response Times}
\label{sec:latency_budget}
A collaborative planning session involves continuous multi-turn dialogue with multiple questions and answers between travelers and the AI (clarifying preferences, asking venue details, requesting swaps, and generating multi-day itineraries). In real-world generative AI workloads, generating full, grounded answers can take from 5 to 20 seconds, especially when synthesizing multiple stops and orchestrating external API calls. Therefore, latency is partitioned between initial perception and complete generation (Table~\ref{tab:latency_budget}).
...
\toprule
\textbf{Interaction \& Query Type} & \textbf{Initial TTFT} & \textbf{Full Answer Time} & \textbf{Operational Context \& Execution Mode} \\
\midrule
\textbf{Room Member Action / Stop Vote} & $< 80$\,ms & $< 80$\,ms & Non-blocking WebSocket ACK + atomic database update. \\
\textbf{Quick Q\&A / Venue Clarifications} & $< 600$\,ms & $3$--$5$\,s & Direct LLM reasoning for queries (parking, dietary, pricing). \\
\textbf{Macro Itinerary Generation} & $< 800$\,ms & $6$--$12$\,s & LangGraph Phase 1 synthesizing multi-day themes and pacing. \\
\textbf{Micro Spot Search \& API Grounding} & $< 1.0$\,s & $10$--$20$\,s & Parallel Google Places calls, opening hours check, and route matrix. \\
\textbf{1-Click Swap / Dynamic Revision} & $< 500$\,ms & $3$--$6$\,s & Fetching verified alternative POIs and recomputing transit. \\
\bottomrule
\end{tabularx}
\vspace{-0.2cm}
\caption{TripMate AI multi-turn interaction latency budget and answer generation times}
\label{tab:latency_budget}
\end{table}

\textbf{Streaming UX for Long Generations:} Because comprehensive planning and live grounding take between 5 and 20 seconds to fully finalize, token-by-token streaming over WebSockets is mission-critical. Travelers observe initial tokens and intermediate reasoning within $< 800$\,ms, allowing the group to read and discuss candidate ideas progressively rather than staring at a frozen loader for 20 seconds.

\subsection{Target Capacity \& Concurrency Sizing}
\label{sec:capacity_concurrency}
TripMate AI's cloud infrastructure is sized to support academic evaluation and active group usage:
\begin{itemize}
\item \textbf{Concurrent Active Users (CCU):} Engineered to reliably serve \textbf{50--100 Concurrent Active Users (CCU)}.
\item \textbf{Collaborative Planning Rooms:} Supports \textbf{10--20 concurrent active rooms} simultaneously (with 3--6 participants actively chatting, voting, and editing in each room).
\item \textbf{Throughput Capacity:} The FastAPI ASGI engine handles \textbf{150--250 requests/sec} for REST and WebSocket traffic under non-blocking asynchronous event loop processing (\texttt{uvloop}).
\item \textbf{Concurrent AI Generations:} Capped at \textbf{5--8 concurrent LangGraph executions}, with excess tasks safely queued in Redis Streams (\texttt{XADD}) to avoid exceeding container memory limits.
\item \textbf{Database Storage:} Supabase's allocation accommodates over \textbf{30,000 completed itineraries} ($\approx 12$--$15$\,KB per full multi-day trip record).
\item \textbf{Connection Multiplexing:} Supavisor connection pooling (port 6543) multiplexes 200+ client sessions into 15 physical PostgreSQL connections.
\end{itemize}

\subsection{Security, Multi-Tenant Privacy \& Data Integrity}
\label{sec:security_nfr}
\begin{itemize}
\item \textbf{Supabase Row Level Security (RLS):} Enforced at the database engine level; queries to \texttt{rooms}, \texttt{messages}, and \texttt{plan\_stops} verify that \texttt{auth.uid()} belongs to registered room members, preventing cross-tenant leakage.
\item \textbf{Atomic Concurrency for Stop Voting:} Simultaneous member votes use atomic SQL updates (\texttt{UPDATE plan\_stops SET upvotes = upvotes + 1 WHERE id = :stop\_id}) and Redis atomic counters, preventing race conditions.
\item \textbf{Prompt Injection Defense:} User chat inputs are sanitized and wrapped in structural XML boundary tags (\texttt{<traveler\_preference>}) with explicit system instructions prohibiting instruction overrides.
\end{itemize}

\subsection{Fault Tolerance \& Checkpoint Recovery}
\begin{itemize}
\item \textbf{LangGraph Checkpoint Persistence:} Workflow execution states are incrementally checkpointed to Supabase via \texttt{AsyncPostgresSaver}, enabling seamless session recovery if the worker restarts.
\item \textbf{Graceful Degradation:} If the primary DeepSeek-V4 API encounters an outage, LangGraph falls back sequentially to OpenAI GPT-4o, Gemini, or a deterministic rule-based local POI generator.
\end{itemize}
