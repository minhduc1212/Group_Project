...
\begin{tikzpicture}[
  userbox/.style={rectangle, draw=primaryblue!80, fill=subtlegray, rounded corners=2mm, line width=0.85pt, minimum width=4.3cm, minimum height=0.82cm, align=center, font=\scriptsize},
  aibox/.style={rectangle, draw=accentteal, fill=badgeteal!40!white, rounded corners=2mm, line width=0.85pt, minimum width=4.5cm, minimum height=0.82cm, align=center, font=\scriptsize},
  ...
  lanehdr/.style={rectangle, draw=primaryblue, fill=primaryblue!10!white, rounded corners=1.5mm, line width=0.9pt, minimum width=4.9cm, minimum height=0.55cm, align=center, font=\footnotesize\bfseries},
...
% 4. Decision 1: Macro OK? (Lane 1) - Ample vertical clearance!
\node[decision] (dec1) at (0, -4.8) {Macro\\OK?};
\draw[actarrow] (act3.south) -- (dec1.north);

% Loopback [Revise Desires] in Left Margin
...
% 7. Decision 2: Stops OK? (Lane 1) - Ample vertical clearance!
\node[decision] (dec2) at (0, -8.6) {Stops\\OK?};
\draw[actarrow] (act5.south) -- (dec2.north);

% 1-Click Swap Alternative POI (Lane 2) - Level with dec2
...
% 9. Live Itinerary Navigation (Lane 1)
...
% Dynamic On-the-Go Adaptation Request (Living Itinerary Loop)
...
\resizebox{\textwidth}{!}{
\begin{tikzpicture}[
  layerbox/.style={rectangle, draw=primaryblue!90, fill=subtlegray, rounded corners=2.5mm, line width=1.0pt, minimum width=4.5cm, minimum height=2.1cm, align=left, font=\footnotesize},
  topbox/.style={rectangle, draw=primaryblue, fill=primaryblue!8!white, rounded corners=2.5mm, line width=1.1pt, minimum width=19.6cm, minimum height=0.95cm, align=center, font=\footnotesize},
  widebox/.style={rectangle, draw=primaryblue!90, fill=subtlegray, rounded corners=2.5mm, line width=1.0pt, minimum width=8.2cm, minimum height=2.4cm, align=left, font=\footnotesize},
...
% Top: Frontend SPA Banner (Full Width 19.6cm)
\node[topbox] (fe) at (0, 5.0) {
\textbf{\large 1. Web Frontend (React 18 SPA)}\\[0.03cm]
\parbox{19.0cm}{\centering \scriptsize
...
\node[layerbox] (be) at (-7.2, 2.0) {
\textbf{\normalsize 2. Backend Gateway (FastAPI)}\\[0.03cm]
\parbox{4.2cm}{\raggedright \scriptsize
...
\node[layerbox, minimum width=3.7cm] (redis) at (0, 2.0) {
\textbf{\normalsize 3. Redis 7 Infrastructure}\\[0.03cm]
\parbox{3.4cm}{\raggedright \scriptsize
...
\node[widebox] (db) at (-5.3, -2.4) {
\textbf{\normalsize 5. Database \& Persistence (PostgreSQL 16)}\\[0.04cm]
\parbox{7.8cm}{\raggedright \scriptsize
...
% Connectors: Bold, Clear, Long Paths with Ample Text Clearances!
% (1) Frontend <-> Gateway
\draw[biarrow] (be.north) -- (be.north |- fe.south) node[midway, left=0.18cm, font=\scriptsize\bfseries, align=right]{\textbf{(1)} WebSockets / REST\\{\tiny(Bidirectional Live Sync)}};

% (2) Task Queue: Gateway -> Redis -> AI Agent (Upper Parallel Arrows)
\draw[flowarrow] ($(be.east)+(0, 0.40)$) -- ($(redis.west)+(0, 0.40)$) node[midway, above=0.04cm, font=\scriptsize\bfseries, text=primaryblue]{\textbf{(2)} Streams};
\draw[flowarrow] ($(redis.east)+(0, 0.40)$) -- ($(ai.west)+(0, 0.40)$) node[midway, above=0.04cm, font=\scriptsize\bfseries, text=primaryblue]{\textbf{(2)} Pull};

% (6) Multi-Instance Broadcast: AI Agent -> Redis -> Gateway (Lower Parallel Arrows)
\draw[flowarrow, color=accentteal] ($(ai.west)-(0, 0.40)$) -- ($(redis.east)-(0, 0.40)$) node[midway, below=0.04cm, font=\scriptsize\bfseries, text=accentteal]{\textbf{(6)} Event};
\draw[flowarrow, color=accentteal] ($(redis.west)-(0, 0.40)$) -- ($(be.east)-(0, 0.40)$) node[midway, below=0.04cm, font=\scriptsize\bfseries, text=accentteal]{\textbf{(6)} Pub/Sub};

% (3) Gateway -> Database (core schema)
\draw[flowarrow] (be.south) -- (be.south |- db.north) node[midway, left=0.18cm, font=\scriptsize\bfseries, align=right]{\textbf{(3)} Transactional Persistence\\{\tiny(Chat, Auth, Rooms, Votes)}};

% (4) AI Agent <-> External APIs (PyBreaker)
\draw[biarrow] (ai.south) -- (ai.south |- ext.north) node[midway, right=0.18cm, font=\scriptsize\bfseries, align=left]{\textbf{(4)} Resilient Tool Queries\\{\tiny(PyBreaker Circuit Breaker)}};

% (5) AI Agent <-> Database (DAL across clear central diagonal corridor)
\draw[biarrow] (ai.south west) -- (db.north east) node[midway, font=\scriptsize\bfseries, fill=white, draw=primaryblue!80, rounded corners=1.2mm, inner sep=2.8pt, align=center]{\textbf{(5)} Data Access Layer (DAL)\\{\tiny(Checkpoints \& \texttt{plan\_stops})}};
