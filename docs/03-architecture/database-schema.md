...
>
> **Cập nhật quan trọng (Kiến trúc Quản lý Tài chính Tối ưu 3 Bảng)**:
> Thay vì sử dụng 6 bảng phức tạp, hệ thống chia tiền (Splitwise Core) đã được rút gọn tối đa xuống 3 bảng: `Expense`, `ExpenseSplit`, và `Settlement`. Thiết kế này vừa tối ưu tốc độ, vừa xử lý được mọi bài toán: thu quỹ trước, chi hộ, và chia tiền không đều (ai chơi nấy chịu).
